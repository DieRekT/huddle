...
async function openTopicSummaryModal({ topic, subtopic, status, start, end }) {
    ...
    try {
        const url = `/api/room/${encodeURIComponent(currentRoom)}/topic-summary?start=${encodeURIComponent(String(start))}&end=${encodeURIComponent(String(end))}`;
        ...
        topicSummaryText.textContent = 'Could not load topic summary. Try again.';
        const li = document.createElement('li');
        li.className = 'empty-state';
...
async function loadTopicHistory(roomCode) {
    ...
    try {
        const r = await fetch(`/api/room/${encodeURIComponent(roomCode)}/topic-history`);
        if (!r.ok) return;
        const j = await r.json();
